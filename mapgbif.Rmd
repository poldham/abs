...
The file containing those results can be found on [Github here](https://github.com/poldham/abs/raw/master/data/kenya_occurrence.rda). 
...
#shinyApp(ui = ui, server = server) # uncomment to run
```
...
server <- function(input, output){
  ...
  # Add summary plot counting species by occurrences. filter(n > 100) to limit swamping the chart. Note that the n value filter will often need adjustment. Probably better to adjust to just slice the top 20 rows here. 
