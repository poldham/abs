...
# add occ_count for use in filtering occurrences
occ_count <- data %>% count(species)
data <- left_join(data, occ_count)
data <- rename(data, "occ_count" = n)
...
server <- function(input, output){
  output$kenya_output <- renderUI({
    ...
    selectInput(inputId = "kingdom_input", "kingdom",
      ...
      selected = "Animalia"),
    sliderInput(inputId = "occ_count", "occurrences", min = min(data$occ_count), max = max(data$occ_count), value = range(data$occ_count))
  )
  })
  ...
  # this is not yet working properly as the points stay the same!
  filtereddata <- reactive({data[data$occ_count >= input$occ_count[1] & data$occ_count <= input$occ_count[2], ]})
  ...
  observe({
    ...
    # Use leafletProxy for elements that change. Note using filteredData here
...
# Add summary plot counting species by occurrences. filter(n > 10) to limit swamping the chart. Note that the n value filter will often need adjustment for the dataset
  output$summary <- renderPlot({
    data %>% 
      filter(data$kingdom %in% input$kingdom_input) %>% 
      count(species) %>%
      dplyr::filter(n > 100) %>% ## Needs to calculate the top 10 or 20 not the numeric value
